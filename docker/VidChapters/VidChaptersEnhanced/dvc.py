...
def train_one_epoch(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        if "output_ap_transits" in batch_dict:
            output_ap_transits = batch_dict["output_ap_transits"].long().to(device)
        else:
            output_ap_transits = None
        ...
        if args.genasr and args.generative:  # vid2seq style generative loss on speech sequence
            ...
            loss_dict, video_dict = model(
                ...
                output_ap_transits=output_ap_transits,
            )
...
def evaluate(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        output, video_attn = model.generate(video=video,
                                input_tokenized=input_tokenized,
        ...
        for i, vid in enumerate(batch_dict["video_id"]):
            ...
            attn[vid] = video_attn[i].cpu()

    all_res = dist.all_gather(res)
    all_attn = dist.all_gather(attn)
    results = reduce(lambda a, b: a.update(b) or a, all_res, {})
    all_attn = reduce(lambda a, b: a.update(b) or a, all_attn, {})
    ...
    if dist.is_main_process():
        if args.save_dir:
            ...
            for vid, attn in all_attn.items():
                attn_path = os.path.join(args.save_dir, dataset_name + f"_{vid}_attn.np",)
                np.save(open(attn_path, 'wb'), attn.numpy())
        else:
        ...
        if dataset_name == "youcook":
            ...
            references = [args.val_json_path if split == 'val' else args.test_json_path]
        else:
...
def main(args):
    ...
    for dset_name in args.combine_datasets:
        ...
        if dset_name in args.combine_datasets_val:
            ...
            if dset_name in ["vitt", "chapters", 'comk']:
