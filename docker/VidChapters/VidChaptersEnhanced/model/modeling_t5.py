...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, encoder_dropout=0., decoder_dropout=0.1, label_smoothing=0.1, is_gated_act=False):
        ...
        encoder_config = copy.deepcopy(config)
        encoder_config.dropout_rate = encoder_dropout
        encoder_config.is_decoder = False
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared)
        ...
        decoder_config = copy.deepcopy(config)
        decoder_config.dropout_rate = decoder_dropout
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # loss_fct = LabelSmoothingCrossEntropy(epsilon=self.label_smoothing, ignore_index=-100)
