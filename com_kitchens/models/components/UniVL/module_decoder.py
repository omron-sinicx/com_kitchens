...
class DecoderConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        if isinstance(vocab_size_or_config_json_file, str):
            with open(vocab_size_or_config_json_file, encoding="utf-8") as reader:
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output, slf_attn_mask=None, dec_enc_attn_mask=None):
        ...
        dec_output, dec_att_scores = self.enc_attn(
            slf_output, enc_output, enc_output, dec_enc_attn_mask
        )
...
class DecoderModel(PreTrainedModel):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight):
        ...
        self.embeddings = DecoderEmbeddings(
            config, decoder_word_embeddings_weight, decoder_position_embeddings_weight
        )
    ...
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None):
        ...
        subsequent_mask = torch.triu(
            torch.ones(
                (len_s, len_s), device=embedding_output.device, dtype=embedding_output.dtype
            ),
            diagonal=1,
        )
        self_attn_mask = (
            subsequent_mask.unsqueeze(0).expand(sz_b, -1, -1).unsqueeze(1)
        )  # b x 1 x ls x ls
