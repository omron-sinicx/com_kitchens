...
from com_kitchens.models.components.xclip.modeling import (
    CLIP4ClipPreTrainedModel,
    check_attr,
    show_log,
    update_attr,
)
from com_kitchens.models.components.xclip.module_clip import CLIP, convert_weights
from com_kitchens.models.components.xclip.module_cross import CrossConfig, CrossModel
from com_kitchens.models.components.xclip.module_cross import Transformer as TransformerClip
from com_kitchens.models.components.xclip.until_module import AllGather, CrossEn, PreTrainedModel
...
class XCLIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        show_log(task_config, f"Stage-One:{self._stage_one}, Stage-Two:{self._stage_two}")
        ...
        if vit:
            ...
            counts: list = [
                len({k.split(".")[2] for k in clip_state_dict if k.startswith(f"visual.layer{b}")})
                for b in [1, 2, 3, 4]
            ]
        ...
        if hasattr(task_config, "sim_header"):
            ...
            show_log(task_config, f"\t sim_header: {self.sim_header}")
    ...
    def forward(
        ...
        **kwargs,
        ...
        (sequence_output, seq_features), visual_output = self.get_sequence_visual_output(
            input_ids,
            token_type_ids,
            attention_mask,
            video,
    ...
    def _loose_similarity(
        ...
        if sim_header == "meanP":
            ...
            visual_output = torch.cat(
                (
                    visual_output,
                    visual_output_original[:, visual_output.size(1) :, ...].contiguous(),
                ),
                dim=1,
            )
        ...
        if self.use_original_clip_for_frame_features:
            frame_features = visual_output_original / visual_output_original.norm(
                dim=-1, keepdim=True
            )  # [bs, num_frames, dim]
        else:
            frame_features = visual_output / visual_output.norm(
                dim=-1, keepdim=True
            )  # [bs, num_frames, dim]
        ...
        word_features = seq_features / seq_features.norm(
            dim=-1, keepdim=True
        )  # [bs, num_words, dim]
        ...
        video_sentence_logits = logit_scale * torch.matmul(
            torch.matmul(sentence_output, self.global_mat_weight), video_output.t()
        )

        # video-word score
        video_word_logits = logit_scale * torch.sum(
            torch.matmul(word_features, video_output.t())
            * torch.matmul(
                torch.softmax(torch.matmul(word_features, video_output.t()) / 1e-2, dim=1).permute(
                    0, 2, 1
                ),
                self.word_logit_weight,
            ).permute(0, 2, 1),
            dim=1,
        )

        # sentence-frame score
        sentence_frame_logits = (
            logit_scale
            * torch.sum(
                torch.matmul(sentence_output, frame_features.permute(0, 2, 1))
                * torch.matmul(
                    torch.softmax(
                        torch.matmul(sentence_output, frame_features.permute(0, 2, 1)) / 1e-2,
                        dim=-1,
                    ),
                    self.frame_logit_weight,
                ),
                dim=-1,
            ).t()
        )
        ...
        # frame-word score
        frame_word_logits = logit_scale * self._attenion_over_fine_grained_sim_matrix(
            word_features, frame_features
        )

        logits = (
            video_sentence_logits + video_word_logits + sentence_frame_logits + frame_word_logits
        ) / 4
    ...
    def _attenion_over_fine_grained_sim_matrix(self, word_features, frame_features):
        ...
        fine_grained_sim_scores = torch.matmul(
            torch.matmul(word_features.view(-1, dim_text), self.local_mat_weight),
            frame_features.view(-1, dim_video).t(),
        ).view(
        ...
        word_level_logit = torch.sum(
            torch.matmul(
                torch.softmax(fine_grained_sim_scores / 1e-2, dim=1).permute(0, 2, 3, 1),
                self.word_mat_weight,
            ).permute(0, 3, 1, 2)
            * fine_grained_sim_scores,
            dim=1,
        )  # [bs_text, bs_video, num_frames]
        frame_level_logit = torch.sum(
            torch.matmul(
                torch.softmax(fine_grained_sim_scores / 1e-2, dim=-1), self.frame_mat_weight
            )
            * fine_grained_sim_scores,
            dim=-1,
        )  # [bs_text, num_words, bs_video]

        sent2frame_logits = torch.sum(
            torch.matmul(torch.softmax(word_level_logit / 1e-2, dim=-1), self.frame_mat_weight2)
            * word_level_logit,
            dim=-1,
        )  # [bs_text, bs_video]
        video2word_logits = torch.sum(
            torch.matmul(
                torch.softmax(frame_level_logit / 1e-2, dim=1).permute(0, 2, 1),
                self.word_mat_weight2,
            ).permute(0, 2, 1)
            * frame_level_logit,
            dim=1,
        )  # [bs_text, bs_video]
    ...
    def _cross_similarity(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        for i in range(len(split_size)):
            ...
            retrieve_logits_row = (
                self.similarity_dense(pooled_output).squeeze(-1).view(step_truth, b_visual)
            )
