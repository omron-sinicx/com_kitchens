...
def _download(url: str, root: str = os.path.expanduser("~/.cache/clip")):
    ...
    with urllib.request.urlopen(url) as source, open(download_target, "wb") as output:
        with tqdm(
            total=int(source.info().get("Content-Length")), ncols=80, unit="iB", unit_scale=True
        ) as loop:
            while True:
...
class CLIP(nn.Module):
    ...
    def get_config(pretrained_clip_name="ViT-B/32"):
        ...
        if pretrained_clip_name in _MODELS and pretrained_clip_name in _PT_NAME:
            model_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), _PT_NAME[pretrained_clip_name]
            )
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        pos_emd = self.positional_embedding[: x.size(1), :].type(
            self.dtype
        )  # ここで77がmaxなのが不味くなっているのか。
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # NLD -> LND
